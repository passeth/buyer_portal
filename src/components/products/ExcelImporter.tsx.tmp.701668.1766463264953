...
import type { ExcelProductRow } from '@/types'
  ...
  const parseExcel = useCallback(async (file: File) => {
    ...
    try {
      ...
      const columnMap: Record<string, string[]> = {
        ...
        base_price_krw: ['본사공급단가(원화)', '본사공급단가', '본사 공급 단가', 'base_price_krw', '단가'],
        volume: ['용량', 'volume', 'size'],
      ...
      const parsed: ParsedProduct[] = jsonData.map((row, index) => {
        ...
        const brand = String(getColumnValue(row, 'brand') || '').trim()
        const category = String(getColumnValue(row, 'category') || '').trim()
        ...
        const volume = String(getColumnValue(row, 'volume') || '').trim()
    ...
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground bg-muted p-4 rounded-lg">
        ...
        <ul className="list-disc list-inside space-y-1">
          <li><strong>품목코드</strong> (필수): 품목코드, 제품코드, product_code</li>
          <li><strong>품목명</strong> (필수): 품목명, 제품명 KOR, name_ko</li>
          <li><strong>브랜드</strong>: 브랜드, brand</li>
          <li><strong>바코드</strong>: 바코드, barcode</li>
          <li><strong>입수량</strong>: pcs / 1 CTN, 입수량</li>
          <li><strong>단가</strong>: 본사공급단가, 단가</li>
        </ul>
      </div>
