  ...
  try {
    ...
    if (body.status && body.status !== existing.status) {
      ...
      if (!allowedNext.includes(body.status)) {
        return NextResponse.json(
          { error: `${existing.status}에서 ${body.status}(으)로 변경할 수 없습니다.` },
          { status: 400 }
        )
      }
    }

    // COMPLETED, CANCELLED 상태에서는 수정 불가 (상태 변경 제외)
    if (['COMPLETED', 'CANCELLED'].includes(existing.status) && !body.status) {
      return NextResponse.json(
    ...
    history.push({
      ...
      action: body.history_action || (body.status ? `상태 변경: ${statusLabels[body.status] || body.status}` : 'updated'),
      by: body.history_by || body.updated_by || 'system',
    ...
    if (body.items && Array.isArray(body.items)) {
      ...
      const newItems = body.items.map((item: {
        ...
        destination: item.destination || body.destination || null,
      ...
      updateData.total_cartons = body.items.reduce((sum: number, item: { qty: number; pcs_per_ctn?: number }) => {
